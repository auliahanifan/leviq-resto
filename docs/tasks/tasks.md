...
- [x] 9.1 Uji end-to-end seluruh alur utama (login → kelola menu → kelola meja → order → bayar → tutup kasir)
- [ ] 9.2 Uji dengan data realistis (semua item menu restoran, jumlah meja aktual 1-10) — perlu data menu & meja asli dari pemilik, belum bisa dikerjakan AI
- [ ] 9.3 Uji di perangkat tablet asli (bukan cuma browser desktop) — cek ukuran tombol & responsivitas — sudah dicek layout responsif di viewport ukuran tablet (768x1024), tapi belum diuji di perangkat fisik
- [ ] 9.4 Deploy ke production, sambungkan domain/URL yang dipakai di tablet restoran — BLOCKED: `SESSION_SECRET` belum diset di environment variables Vercel sehingga login production saat ini error (lihat catatan di bawah); domain kustom juga belum disambungkan
- [ ] 9.5 Pelatihan singkat kasir menggunakan aplikasi (walkthrough semua fitur) — aksi manusia, tidak bisa dikerjakan AI
- [ ] 9.6 Monitoring 1 minggu pertama pemakaian: pastikan 0% kembali ke nota manual, tutup kasir < 5 menit, tidak ada data transaksi hilang (metrik di PRD §11) — perlu pemakaian nyata dulu, belum bisa dimulai
...
- **Action item production (blocker 9.4):** tambahkan environment variable `SESSION_SECRET` di Vercel (Project Settings → Environment Variables, untuk Production). Generate nilainya dengan `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`, lalu redeploy. Tanpa ini, login di production akan selalu gagal (dikonfirmasi lewat Vercel runtime error log: 18 error "SESSION_SECRET belum diset" sejak Fase 2 live). RLS permisif pada tabel `tables`/`menu_items`/`orders`/`order_items`/`cash_closings` (dicatat di migrasi 1.7) juga masih jadi item hardening yang belum dikerjakan.
