...
- [ ] 9.4 Deploy ke production, sambungkan domain/URL yang dipakai di tablet restoran — Deploy production sudah jalan (`SESSION_SECRET` sudah diset & login sudah diverifikasi sukses langsung di leviq-resto.vercel.app), tapi domain kustom untuk tablet restoran belum disambungkan
...
- **Resolved:** `SESSION_SECRET` sudah ditambahkan di Vercel (Production + Preview, sensitive) dan project sudah di-redeploy. Diverifikasi langsung: login PIN 1234 di https://leviq-resto.vercel.app berhasil masuk ke Daftar Meja, dan Vercel runtime error log bersih (sebelumnya 18 error "SESSION_SECRET belum diset" sejak Fase 2 live). Masih jadi item terbuka: domain kustom belum disambungkan, dan RLS permisif pada tabel `tables`/`menu_items`/`orders`/`order_items`/`cash_closings` (dicatat di migrasi 1.7) masih jadi kandidat hardening berikutnya.
